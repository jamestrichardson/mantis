...
Every fixture-backed tool here reuses an existing builder from #28/#8/#9/#10's
own fixture modules (``mantis.eval.fixtures.awx``/``network``/``prometheus``/
``loki``) -- no new client or tool logic is introduced, only new canned
data wired through those existing builders.

Three scenarios, matching #11's acceptance criteria:
...
def _register_secondary_tools(registry: ToolRegistry) -> None:
    """Register the two allowlisted tools (``awx_recent_failed_jobs``,
    ``prometheus_query``) that none of this module's golden scenarios
    require calling, but which must still resolve: ``ALLOWED_TOOLS``
    (all six real System Troubleshooter tools) is what every scenario
    here passes as ``agent_tools``, and ``AgentRuntime.__post_init__``
    calls ``registry.subset(self.tools)``, which raises
    ``ToolNotFoundError`` if *any* named tool isn't registered in the
    scenario-scoped registry -- not just the ones a given scenario's
    golden path actually exercises. A model remains free to call either
    of these; they simply aren't required for a correct answer to any of
    this module's specific questions.
    """
