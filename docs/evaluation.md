...
because `run_scenario` itself already isolates a model/backend failure
as an `outcome="error"` record and never raises for one.
`qualify_models` preserves that exact isolation boundary and does not
widen it: an unexpected exception (a bug in a scenario's fixture, a
baseline/registry drift, a scoring bug) is a genuine Mantis defect, not
model evidence, and propagates out of `qualify_models` exactly as it
would out of `run_scenario`, invalidating the whole qualification run
rather than being silently attributed to whichever model happened to
be running.
...
suite id/version; outcome (`ok`/`error`); `passed`/`score`/
