...
```bash
# mantis run always requires an explicit prompt -- there is no
# no-argument/default-prompt invocation through the API or CLI
# (RunRequest.prompt is required; AgentCatalogEntry.default_prompt is
# only ever used by the unsupported direct-module escape hatch below).
mantis run incident-triage "Investigate the incident affecting ferros-c01 between 2026-09-16T02:55:00+00:00 and 2026-09-16T03:15:00+00:00."

# Or with a different target and window:
...
# the API/catalog, see docs/agents.md#what-constitutes-an-agent). This
# one alone falls back to DEFAULT_PROMPT (the same worked example above)
# when no prompt argument is given -- mantis run itself never does:
python -m mantis.agents.incident_triage
...
running the real tool functions against fixture data). Note that the
prompt never states a job id: a grounded investigation calls
`awx_recent_failed_jobs(limit=5)` first to discover job `7301` exists,
then `awx_get_job_failure(job_id=7301)` for its structured detail — the
golden scenario requires both calls, in that order, for exactly this
reason.
...
See `mantis.eval.fixtures.incident_triage` for six more golden
...
(a timestamped Kubernetes `Warning`/`BackOff` event bracketed by a
Prometheus scrape gap, alongside current pod state that must not be
read back onto the incident window), `incident-triage-untrusted-kubernetes-event`
...
(the input contract's other half: an incident target with no window,
and a window with no target — golden behavior is to ask and make zero
tool calls in both cases) — and
`tests/eval/test_incident_triage_scenarios.py` for the deterministic
good/bad-answer scoring tests, including fixture-coherence tests that
directly assert every scenario's canned evidence timestamps actually
fall inside its own declared incident window.
...
- **No dynamic model routing** (tracked separately, #16) — model
  selection today is a single configured `LITELLM_MODEL`/
  `MANTIS_INCIDENT_TRIAGE_MODEL` alias.
