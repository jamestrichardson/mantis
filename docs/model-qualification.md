...
`mantis.eval.fixtures.incident_triage`) and before the `#16` routing
fields (`final_alias`/`failed_route_attempts`) existed on
`QualificationRecord`. Presenting that run's numbers against the
